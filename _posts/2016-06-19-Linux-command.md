...
说明: 在shell脚本中,默认情况下无法使用alias命令对其他命令进行重命名
```
...
注： 
1)当出现vdi等格式的img文件时可能无法kpartx，这个时候可以使用qemu-img convert命令把起格式转换为raw格式，然后再尝试。
...
Example：(访问193.168.246.254:80端口会被转发到193.168.197.232:80上)
```
...
注：
-exec： find命令对匹配的文件执行给出的shell命令（如：find . -type f -exec ls -l { }  \;）
